@interface MenstrualCyclesAppPlugin.InternalLunaLiveOnSettingsModel : NSObject <HKMCAnalysisProviderObserver, HKMCSettingsManagerObserver, HKSPSleepStoreObserver> {
    void /* unknown type, empty encoding */ settingsManager;
    void /* unknown type, empty encoding */ healthStore;
    void /* unknown type, empty encoding */ analysisProvider;
    void /* unknown type, empty encoding */ analysis;
    void /* unknown type, empty encoding */ menstrualCyclesStatusManager;
    void /* unknown type, empty encoding */ wristTemperatureMeasurementsStatusManager;
    void /* unknown type, empty encoding */ wristTemperatureInputStatusManager;
    void /* unknown type, empty encoding */ sleepMeasurementConfigurationProvider;
    void /* unknown type, empty encoding */ sleepMeasurementConfiguration;
    void /* unknown type, empty encoding */ sleepStore;
    void /* unknown type, empty encoding */ _requirements;
    void /* unknown type, empty encoding */ subscriptions;
}

- (void)settingsManagerDidUpdateAnalysisSettings:(id)a0;
- (void)sleepStore:(id)a0 sleepScheduleDidChange:(id)a1;
- (void)sleepStore:(id)a0 sleepSettingsDidChange:(id)a1;
- (void)analysisProvider:(id)a0 didUpdateAnalysis:(id)a1;
- (id)init;
- (void).cxx_destruct;

@end
