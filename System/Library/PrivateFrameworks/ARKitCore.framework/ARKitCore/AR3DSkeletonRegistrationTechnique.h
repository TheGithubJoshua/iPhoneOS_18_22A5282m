@class ABPKCameraRegistration, ABPKDepthBasedScaleEstimation, ARWorldTrackingPoseData;
...
@interface AR3DSkeletonRegistrationTechnique : ARTechnique {
    ...
    struct array<arkit::btr::RegistrationData, 5UL> { struct RegistrationData { struct Matrix<float, 2U, 16U, false> { float m_data[32]; } image_points; struct Matrix<float, 3U, 16U, false> { float m_data[48]; } lifted_points; struct Matrix<float, 3U, 3U, false> { float m_data[9]; } intrinsics; struct SE3GroupStorage<float, cva::Matrix<float, 4, 4, false>> { float m_data[16]; } cam_from_obj; struct SE3GroupStorage<float, cva::Matrix<float, 4, 4, false>> { float m_data[16]; } cam_from_vio; double timestamp_obj; double timestamp_vio; } __elems_[5]; } _last_btr_poses;
    struct basic_string<char, std::char_traits<char>, std::allocator<char>> { struct __compressed_pair<std::basic_string<char>::__rep, std::allocator<char>> { struct __rep { union { struct __long { char *__data_; unsigned long long __size_; unsigned long __cap_ : 63; unsigned char __is_long_ : 1; } __l; struct __short { char __data_[23]; unsigned char __padding_[0]; unsigned char __size_ : 7; unsigned char __is_long_ : 1; } __s; struct __raw { unsigned long long __words[3]; } __r; } ; } __value_; } __r_; } _dumps_dir;
